...
import {
  ...
  TouchableOpacity, StatusBar, Modal, Alert, Platform, ActivityIndicator,
} from "react-native";
...
const AlertThumb = ({ item, onPress }) => (
  <TouchableOpacity style={styles.alertThumb} onPress={onPress} activeOpacity={0.8}>
    <View style={styles.alertThumbImg}>
      {item.image_url ? (
        <Image source={{ uri: item.image_url }} style={styles.alertThumbPhoto} />
      ) : null}
...
useEffect(() => {
  ...
  const fetchByIP = async () => {
    try {
      ...
      const res = await fetch("http://ip-api.com/json/?fields=lat,lon,city&lang=vi");
  ...
  if (error || !weather) {
    return (
      <View style={styles.card}>
        <Text style={styles.cardSectionTitle}>{'DỰ BÁO THỜI TIẾT'}</Text>
        <Text style={styles.weatherError}>{'Không lấy được dữ liệu thời tiết'}</Text>
      </View>
    );
  }
...
export default function HomeScreen({ navigation }) {
  ...
  const cameraCandidates = useMemo(() => getCameraBaseCandidates(), []);
  ...
  const socketCandidates = useMemo(() => getSocketBaseCandidates(), [cameraBaseUrl]);
  ...
  const showIntrusionToast = useCallback(() => {
    ...
    toastTimerRef.current = setTimeout(() => {
      Animated.timing(toastAnim, { toValue: 0, duration: 220, useNativeDriver: true })
        .start(({ finished }) => { if (finished) setToastVisible(false); });
      toastTimerRef.current = null;
    }, 2400);
  }, [toastAnim]);

  const fetchRecentAlerts = useCallback(async () => {
    if (!user) { setRecentAlerts(MOCK_ALERTS); return; }
    try {
    ...
    const probeCamera = async () => {
      ...
      for (const baseUrl of cameraCandidates) {
        try {
          ...
          const res = await fetch(`${baseUrl}/viewer/camera`, { method: "GET", signal: controller.signal });
          clearTimeout(timer);
          if (res.ok) { setCameraBaseUrl(baseUrl); setCameraLoading(false); return; }
        } catch {}
      }
      setCameraError("Không kết nối được camera server.");
  ...
  useEffect(() => {
    ...
    const connectSocket = async () => {
      for (const baseUrl of socketCandidates) {
        ...
        activeSocket = socket;
        socket.on("new_alert", () => { showIntrusionToast(); fetchRecentAlerts(); });
        return;
      }
  ...
  }, [cameraLoading, fetchRecentAlerts, showIntrusionToast, socketCandidates, socketOptions]);
  ...
  const viewerUrl = cameraBaseUrl ? getViewerUrl(cameraBaseUrl, "Camera chinh") : null;
  const requireAuth = (action) => { if (!user) navigation.navigate("Login"); else action?.(); };
...
<View style={styles.dropdownUser}>
  ...
  <View>
    ...
    <Text style={styles.dropdownUserEmail}>{user?.email || ''}</Text>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scroll}>
        ...
        <View style={styles.cameraCard}>
          ...
          <View style={styles.cameraFooter}>
            ...
            <Text style={styles.cameraFooterTime}>{new Date().toLocaleTimeString('vi-VN', { hour: '2-digit', minute: '2-digit' })}</Text>
          </View>
        </View>
      ...
      <TouchableOpacity
        activeOpacity={0.88}
        style={[styles.alertStatusBtn, { backgroundColor: hasActiveAlarm ? ALERT_BUTTON_ACTIVE : ALERT_BUTTON_IDLE }]}
        onPress={() => requireAuth(() => { setHasActiveAlarm(false); navigation.navigate("Alerts"); })}
      >
        <View style={styles.alertStatusInner}>
          <View style={[styles.alertStatusDot, { backgroundColor: hasActiveAlarm ? "#FCA5A5" : "#86EFAC" }]} />
...
const styles = StyleSheet.create({
  ...
  // Toast
  alertToast: {
    position: "absolute", top: 10, right: 12, zIndex: 2000,
    backgroundColor: "#DC2626", borderRadius: 12,
    paddingHorizontal: 16, paddingVertical: 10, maxWidth: "88%",
    shadowColor: "#000", shadowOpacity: 0.2, shadowRadius: 8, elevation: 6,
  },
  alertToastText: { color: "#FFF", fontSize: 13, fontWeight: "800", letterSpacing: 0.4 },
  ...
  headerTitle: { fontSize: 16, fontWeight: "800", color: TEXT1, letterSpacing: 0.8 },
  ...
  cameraStateHint: { color: "rgba(203,213,225,0.6)", fontSize: 11, textAlign: "center" },
  cameraFooter: {
    flexDirection: "row", alignItems: "center",
    paddingHorizontal: 14, paddingVertical: 10, gap: 6,
  },
  cameraFooterDot: { width: 6, height: 6, borderRadius: 3, backgroundColor: "#22C55E" },
  ...
  // Thời tiết
  weatherLoading: { flexDirection: "row", alignItems: "center", gap: 10, paddingVertical: 8 },
  ...
  weatherLeft: { flex: 1 },
  weatherCity: { fontSize: 13, fontWeight: "600", color: TEXT2, marginBottom: 8 },
  weatherTempRow: { flexDirection: "row", alignItems: "center", gap: 12, marginBottom: 10 },
  weatherSymbol: { fontSize: 42, lineHeight: 48 },
  weatherTemp: { fontSize: 32, fontWeight: "800", color: TEXT1, lineHeight: 36 },
  ...
  weatherMetaDot: { width: 3, height: 3, borderRadius: 2, backgroundColor: TEXT3 },
  weatherForecast: {
    flexDirection: "row", gap: 6,
    borderLeftWidth: 1, borderLeftColor: BORDER, paddingLeft: 16,
  },
  forecastItem: {
  ...
  alertThumbImg: {
    height: 68, backgroundColor: "#EEF2F7",
    borderRadius: 12, overflow: "hidden",
    justifyContent: "flex-end", padding: 6, marginBottom: 5,
  },
  alertThumbPhoto: { ...StyleSheet.absoluteFillObject, width: "100%", height: "100%" },
  alertThumbTimeBox: {
  ...
  alertStatusInner: { flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 8 },
  alertStatusDot:   { width: 8, height: 8, borderRadius: 4 },
  alertStatusBtnText: { color: "#FFF", fontSize: 13, fontWeight: "800", letterSpacing: 0.6 },
  ...
  dropdownMenu: {
    ...
    shadowColor: "#000", shadowOpacity: 0.14, shadowRadius: 16, elevation: 10,
    overflow: "hidden",
  },
  dropdownUser: {
    flexDirection: "row", alignItems: "center",
