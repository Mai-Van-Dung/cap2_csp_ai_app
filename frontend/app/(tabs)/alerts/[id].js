      ...
      <View style={styles.card}>
        <View style={styles.cardHeader}>
          ...
          <View style={styles.badgeRow}>
            ...
            {pinned && <Pill label="Pinned" tone="info" />}
          </View>
        </View>
        <Text style={styles.meta}>{alert.cameraName} · {alert.timeAgo || relativeTimeFromNow(alert.createdAt)}</Text>
        ...
        <TouchableOpacity style={styles.pinBtn} onPress={() => setPinned((p) => !p)}>
          <Text style={styles.pinText}>{pinned ? 'Unpin' : 'Pin alert'}</Text>
        </TouchableOpacity>
      </View>
...
const styles = StyleSheet.create({
  ...
  pinText: { color: '#0369A1', fontWeight: '700' },
});
