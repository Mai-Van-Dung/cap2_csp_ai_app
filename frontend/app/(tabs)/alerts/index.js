...
const FILTERS = ['all', 'open', 'resolved', 'high', 'medium', 'low'];
...
export default function AlertsList() {
  ...
  const filtered = useMemo(() => {
    ...
    if (['high', 'medium', 'low'].includes(filter)) {
      return mockAlerts.filter((a) => a.severity === filter);
    }
        ...
        {FILTERS.map((f) => (
          ...
          >
            <Text style={[styles.chipText, filter === f && styles.chipTextActive]}>
              {f === 'all' ? t('filtersAll')
                : f === 'open' ? t('filtersOpen')
                : f === 'resolved' ? t('filtersResolved')
                : f === 'high' ? t('filtersHigh')
                : f === 'medium' ? t('filtersMedium')
                : t('filtersLow')}
            </Text>
          </TouchableOpacity>
        ))}
          ...
          <TouchableOpacity style={styles.card} onPress={() => router.push(`/alerts/${item.id}`)}>
            <View style={styles.cardRow}>
              <View style={{ flex: 1 }}>
                ...
                <Text style={styles.cardMeta}>{item.cameraName} · {item.timeAgo || relativeTimeFromNow(item.createdAt)}</Text>
              </View>
...
const styles = StyleSheet.create({
  ...
  chipActive: { backgroundColor: '#2E7D8C' },
